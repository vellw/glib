 ...
 * projects. Since GLib 2.66, this can be implemented by dropping messages
...
static gboolean gmessages_prefer_stderr = FALSE;

/**
 * g_log_writer_default_set_prefer_stderr:
 * @prefer_stderr: If %TRUE, use `stderr` for log messages that would
 *  normally have appeared on `stdout`
 *
 * Configure whether the default GLib log writer will prefer to output
 ...
 */
void
g_log_writer_default_set_prefer_stderr (gboolean prefer_stderr)
{
  g_return_if_fail (g_thread_n_created () == 0);
  gmessages_prefer_stderr = prefer_stderr;
}
...
mklevel_prefix (gchar          level_prefix[STRING_BUFFER_SIZE],
                ...
                gboolean       use_color)
{
  gboolean to_stdout = !gmessages_prefer_stderr;
